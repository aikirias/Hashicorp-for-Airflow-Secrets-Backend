...
FILE_PATH="${1:-}"
TOKEN_FILE="${VAULT_AGENT_TOKEN_FILE:-/vault-agent/output/agent-token}"
...
if [ -n "${VAULT_TOKEN:-}" ]; then
  ...
  echo "[vault-agent] no token available (file: ${TOKEN_FILE})" >&2
  exit 1
fi
...
printf '%s\n' "[vault-agent] publishing dynamic conn_id vault_agent_postgres"
